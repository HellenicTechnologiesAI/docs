...
| Redis | **deleted**. Snapshot: `omniai-redis-stg-before-off-20260903` |
| Task def file | `.aws/task-definition-stage.json` |
| ECS desired count | **0** / running **0** |

Cloudflare `staging` no longer points at AWS. Public `staging.omniaichatbot.com` is the new Droplet.

## GitHub Actions trap

`.github/workflows/build_and_push_aws.yml` is **manual-only**. Do not run it. Live customers are not on ECS. After this cut, GitHub secrets `AWS_SECRET_ID` / `AWS_ACCESS_KEY` can be removed.
